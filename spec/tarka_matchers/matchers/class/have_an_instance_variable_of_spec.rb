			...
			context 'when class is baz' do
				...
				context 'when instance_variable is :@tazzy' do
					...
					context "when expected is ['wow','woz','zooper']" do
						...
						it{ is_expected.to fail }
						it{ is_expected.to have_a_failure_message_of "failed to contain an instance variable called, '#{instance_variable}', that equals '#{escape(expected.to_s)}'.#{difference_format}" }
					end
				end
